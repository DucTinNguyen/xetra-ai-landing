...
const BtnNormal = ({ title, className }: { title: string, className: string }) => {
  return (
      ...
      <span className='w-full h-full group-hover:bg-black group-hover:text-white transition-all ease-linear duration-100  bg-white font-medium text-sm rounded-lg font-inter flex items-center justify-center'>{title}</span>
      </button>
  )
}
